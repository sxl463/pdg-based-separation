...
bool DataDependencyGraph::runOnFunction(llvm::Function &F)
{
  ...
  for(inst_iterator instIt = inst_begin(F), E = inst_end(F); instIt != E; ++instIt){
	...
	DDG->addDependency(InstructionWrapper::instMap[flowdep_set[i]], InstructionWrapper::instMap[pInstruction], DATA_RAW);
      }
...
static RegisterPass<DataDependencyGraph> DDG("ddg", "Data Dependency Graph Construction", false, true);
